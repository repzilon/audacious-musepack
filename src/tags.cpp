...
void tag_insert ( char* buffer, const char* value, long unsigned int  len, long unsigned int  maxlen, bool decode_utf8 )
{
    char* p;
	...
	if ( (c = wcsrtombs(temp,&src,MAX_LEN,NULL)) ==0) return;
    } else {
        c = len;
...
int ReadID3Tag ( VFSFile* fp, ape_tag* Tag )
{
	char * tag;
	char * buff;
	unsigned char genre;
	...
	if(genre>=sizeof(GenreList)/sizeof(int)) genre = 12;
...
// Reads APE v2.0 tag
int ReadAPE2Tag ( VFSFile* fp, ape_tag* Tag )
{
    unsigned long               vsize;
    unsigned long               isize;
    unsigned long               flags;
        ...
        if ( isize > 0 && vsize > 0 ) {
            if ( !(flags & 1<<1) ) {   // insert UTF-8 string (skip binary values)
                if      ( !strcasecmp ((char *)p, "Title") ) {
                    tag_insert ( Tag->title, (char *)(p+isize+1), vsize, MAX_LEN, false);
                }
                else if ( !strcasecmp ((char *)p, "Artist") ) {
                    tag_insert ( Tag->artist, (char *)(p+isize+1), vsize, MAX_LEN, false );
                }
                else if ( !strcasecmp ((char *)p, "Album") ) {
                    tag_insert ( Tag->album, (char *)(p+isize+1), vsize, MAX_LEN, false);
                }
                else if ( !strcasecmp ((char *)p, "Comment") ) {
                    tag_insert ( Tag->comment, (char *)(p+isize+1), vsize, MAX_LEN, false);
                }
                else if ( !strcasecmp ((char *)p, "Genre") ) {
                    tag_insert ( Tag->genre, (char *)(p+isize+1), vsize, MAX_LEN, false );
                }
                else if ( !strcasecmp ((char *)p, "Track") ) {
                    tag_insert ( Tag->track, (char *)(p+isize+1), vsize, 128, false );
                }
                else if ( !strcasecmp ((char *)p, "Year") ) {
                    tag_insert ( Tag->year, (char *)(p+isize+1), vsize, 128, false );
                }
            }
        }
        p += isize + 1 + vsize;
...
int DeleteTag (char* filename)
{
  ...
  if (fp== NULL)
  {
  	char  text [256];
        ...
        xmms_show_message ( "File-Error", (gchar*)text, "Ok", FALSE, NULL, NULL );
	return -1;
  }
  ...
  {
	res = vfs_truncate(fp,(off_t)(filelength-dellength));
  }  
  vfs_fclose(fp);
  free(tagheader);
  free(apelength);
...
int addValue(TagItem* item,char* key,char* value)
{
...
int WriteAPE2Tag ( char* filename, ape_tag *Tag )
{
  VFSFile *fp;
  ...
  if (tagtype!=TAG_NONE)
  	if (DeleteTag(filename)!=0) return 0;
